...
base_sin_outliers = copia_seguridad[!(rownames(copia_seguridad) %in% c(rownames(potenciales_outliers),"76")),]
...
Es por ello que podemos concluir que el mjor ajuste es con direccion de 45 grados, dado que es el que mas se aproxima a la distribucion al avariograma empirico.
...
vt_exp = fit.variogram(v, vgm(170000, "Exp", 1000/3, 40000))
rango_practico = 300 * 3
plot(v , vt_exp, main = 'Variograma Teórico Exponencial sin tendencia')
```

```{r}
vt_sph = fit.variogram(v, vgm(170000, "Sph", 1000/3, 40000))
...
```{r}
vtent_exp = fit.variogram(vten, vgm(170000, "Exp", 1000/3, 40000))
plot(vten , vtent_exp, main = 'Variograma Teórico Exponencial con tendencia')
```

```{r}
vtent_sph = fit.variogram(vten, vgm(170000, "Sph", 1000/3, 40000))
...
```{r}
vExp <- fit.variogram(v, vgm(170000,model = "Exp", 1000/3, 40000),fit.method = 2)
vSph <- fit.variogram(v, vgm(170000,model = "Sph", 1000/3, 40000),fit.method = 2)
vMat <- fit.variogram(v, vgm(170000,model = "Mat", 1000/3, 40000),fit.method = 2)
vBes <- fit.variogram(v,vgm(170000,model="Bes", 1000/3, 40000),fit.method = 2)
vSte <- fit.variogram(v,vgm(170000,model="Ste", 1000/3, 40000),fit.method = 2)


vExpLine=variogramLine(vExp,1400)
vSphLine=variogramLine(vSph,1400)
vMatLine=variogramLine(vMat,1400)
vSteLine=variogramLine(vSte,1400)
vBesLine=variogramLine(vBes,1400)
...
```{r}
vv <- variogram(zinc~1, base_sin_outliers, cutoff = 4200, width = 200, map=T)
plot(vv)
```

```{r}
vv <- variogram(zinc~1, base_sin_outliers, cutoff = 1500, width = 100)
plot(vv)
```
...
media_zinc = mean(base_sin_outliers$zinc)
```

```{r}
# Modelo Exponencial
k1_ord <- krige(zinc~1, base_sin_outliers, meuse.grid, model = vt_exp, nmax = 155, beta = media_zinc)
# Modelo Esférico
k2_ord <- krige(zinc~1, base_sin_outliers, meuse.grid, model = vt_sph, nmax = 155, beta = media_zinc)
```
...
```{r}
#Modelo Exponencial
valcruz1_ord <- krige.cv(zinc~1, base_sin_outliers, vt_exp, nfold=155)
#Modelo Esférico
valcruz2_ord <- krige.cv(zinc~1, base_sin_outliers, vt_sph, nfold=155)
```
...
```{r}
par(mfrow=c(1,2))
plot(valcruz1_ord$observed,valcruz1_ord$observed - valcruz1_ord$residual,xlab="Observados (Exp)", ylab="Predichos (Exp)", col = 'cadetblue')
abline(a = 0, b = 1, col = 'red')
plot(valcruz2_ord$observed,valcruz2_ord$observed - valcruz2_ord$residual,xlab="Observados (Sph)", ylab="Predichos (Sph)", col = 'cadetblue')
abline(a = 0, b = 1, col = 'red')
par(mfrow=c(1,1))
```
